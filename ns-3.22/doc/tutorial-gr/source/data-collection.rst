...
Το τελευταίο κεφάλαιο του οδηγού μας εισάγει κάποια συστατικά μέρη που προστέθηκαν
...
εξόδου, είτε για ερευνητικούς σκοπούς ή απλά για την εκμάθηση του συστήματος.
Στο προηγούμενο κεφάλαιο, εισαγάγαμε το υποσύστημα ιχνηλάτησης και το παράδειγμα
``sixth.cc`` από το οποίο παράγονται PCAP ή ASCII αρχεία ιχνών. Αυτά τα ίχνη
είναι πολύτιμα για ανάλυση δεδομένων με χρήση ποικιλίας εξωτερικών εργαλείων, και
...
από την απλή δημιουργία αρχείων ιχνηλάτησης, συμπεριλαμβανομένων και των ακόλουθων:
...
* δημιουργία δεδομένων που δεν καταγράφονται καλά σε ίχνη PCAP ή ASCII, όπως
  είναι τα δεδομένα πέρα των πακέτων (π.χ. μεταβάσεις καταστάσεων μηχανής σύμφωνα
  με πρωτόκολλα)
* μεγάλες προσομοιώσεις για τις οποίες οι απαιτήσεις εισόδου-εξόδου σε χωρητικότητα
  για τη δημιουργία αρχείων ιχνηλάτησης είναι απαγορευτικές ή επιβαρυντικές, και
* η ανάγκη για *online* αναγωγή δεδομένων ή υπολογισμό, κατά τη διάρκεια της
  εκτέλεσης της προσομοίωσης. Ένα καλό παράδειγμα σχετικά με αυτό είναι ο ορισμός
  μια τερματικής συνθήκης για την προσομοίωση, ώστε να της πείτε πότε να σταματήσει
  όταν έχει λάβει αρκετά δεδομένα ώστε να σχηματίσει ένα αρκετά-στενό διάστημα 
  εμπιστοσύνης γύρω από μια εκτίμηση κάποιας παραμέτρου.
...
επιπρόσθετες ικανότητες πέρα από αποτελέσματα που βασίζονται σε ίχνη. Συνιστούμε
...
μέσω ενός παραδείγματος κάποιες από τις ικανότητες ανάπτυξης.
...
IPv6 αντί του IPv4. Η επιλογή ``help``, διαθέσιμη σε όλα τα προγράμματα του |ns3|
...
μπορεί να αλλάξει μέσω εναλλαγής της δυαδικής τιμής ως ακολούθως:
...
και ρίξτε μια ματιά στο PCAP που έχει δημιουργηθεί, για παράδειγμα με την
εντολή ``tcpdump``:
...
Αυή ήταν μια μικρή παρέκβαση σχετικά με την υποστήριξη του IPv6 και της
γραμμής εντολών, η οποία επίσης παρουσιάστηκε νωρίτερα σε αυτόν τον οδηγό. Για 
...
`seventh.cc`` είχε δημιουργήσει έναν αριθμό από νέα αρχεία εξόδου:
...
Αυτά δημιουργήθηκαν από πρόσθετες δηλώσεις που εισήχθησαν παραπάνω. Πιο συγκεκριμένα,
από έναν GnuplotHelper και έναν FileHelper. Αυτά τα δεδομένα παρήχθησαν προσδένοντας
τα μέρη για τη συλλογή δεδομένων σε πηγές ιχνών του |ns3|, και μέσω τοποθέτησης των
δεδομένων σε ένα διαμορφωμένο ``gnuplot`` και σε ένα διαμορφωμένο αρχείο κειμένου.
...
για συνήθεις περιπτώσεις. Συνδέει πηγές ιχνών του |ns3| με τύπους δεδομένων που 
υποστηρίζονται από το σύστημα συλλογής δεδομένων. Δεν υποστηρίζονται όλοι οι τύποι
δεδομένων που αφορούν τις πηγές ιχνών του |ns3|, αλλά πολλοί από τους συνηθισμένους
τύπους ιχνηλάτησης υποστηρίζονται, συμπεριλαμβανομένου των TracedValues μαζί με
...
διαχωρισμένες-με-κενά και καταμετρήσεις byte των πακέτων. Θα καλύψουμε παρακάτω 
το πως η συγκεκριμένη έξοδος δεδομένων καθορίστηκε, αλλά τώρα ας συνεχίσουμε με τα
...
έξοδο με το όνομα ``seventh-packet-byte-count.png``. Τέλος, ένα μικρό σενάριο κέλυφους
``seventh-packet-byte-count.sh`` εκτελεί αυτό το αρχείο γραφικής παράστασης μέσω του gnuplot
για να παράξει το επιθυμητό PNG (το οποίο μπορείτε να δείτε μέσω ενός επεξεργαστή εικόνων).
...
"Καταμέτρηση Byte Πακέτων προς Χρόνο", και ότι σχεδιάζει τα δεδομένα που έχουν
ανιχνευθεί σε αντιστοιχία με το μονοπάτι της πηγής ιχνηλάτησης: 
...
μεγάλο βαθμό 596-byte TCP πακέτα στη μια κατεύθυνση, και 60-byte TCP επιβεβαιώσεις
στην άλλη (πηγές ιχνών δύο κόμβων αντιστοιχήθηκαν σε αυτή την πηγή ιχνών). 
...
Τα πρώτα δύο ορίσματα είναι το όνομα του probe τύπου και το μονοπάτι της πηγής ιχνών.
Αυτά τα δύο είναι πιθανώς τα δυσκολότερα προς ορισμό όταν προσπαθείτε να χρησιμοποιήσετε
αυτό το πλαίσιο εργασίας για να σχεδιάσετε άλλα ίχνη. Το probe ίχνος εδώ είναι η ``Tx``
πηγή ιχνών της κλάσης ``Ipv6L3Protocol``. Όταν εξετάσουμε την υλοποίηση αυτής της κλάσης
...
Probe κλάση (αυτό που χρειαζόμαστε εδώ) της κλάσης Ipv6PacketProbe. Δείτε τα
...
Ο Ipv6PacketProbe εξάγει, ο ίδιος, μερικές πηγές ιχνών που εξάγουν τα δεδομένα
...
"OutputBytes" του Ipv6PacketProbe.
Τέλος, τα δύο τελευταία ορίσματα της δήλωσης παρέχουν το υπόμνημα της παράστασης
...
Οι ακόλουθες καταγεγραμμένες τιμές υποστηρίζονται μαζί με Probes έως και τη στιγμή που γράφεται
το παρόν κείμενο: 
...
Οι ακόλουθοι τύποι TraceSource υποστηρίζονται από τους Probes έως και τη στιγμή που γράφεται
το παρόν κείμενο: 
...
προς την εξαγωγή του μεγέθους του Packet (σε byte). Ωστόσο, οι περισσότεροι
...
προσδιοριστής διαμόρφωσης είναι το επόμενο. Κάποιες άλλες επιλογές διαμόρφωσης 
...
Εν τέλει, η πηγή ιχνών που μας ενδιαφέρει πρέπει να προσδεθεί. Πάλι, χρησιμοποιούνται
...
έχει προστεθεί η βασική υποστήριξη για την παροχή αποτελεσμάτων σε χρονικές σειρές.
